...
def _parse_args():
    ...
    parser.add_argument("--input", required=True, help="Cleaned travel workbook path")
...
def main():
    ...
    neg_km.to_excel(output_dir / 'negative_km_rows.xlsx', index=False)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'travels_by_cost_center.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'monthly_co2.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'co2_exists_pie_chart.png')
    plt.close()
